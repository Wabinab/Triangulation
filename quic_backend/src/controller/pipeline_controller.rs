...
/// We clone this from modify_kelly. 
fn modify_checklist(data_path: PathBuf, msg: Bytes, crud: CRUD) -> Result<Option<String>, String> {
  let submit: SubmitChecklist = serde_json::from_slice(&msg).unwrap();
  let old_serde = get_data(data_path.clone(), submit.filename.clone());
  if old_serde.is_err() { error!("modify_checklist old_serde"); return Err(old_serde.unwrap_err()); }
  ...
  let ret = compress_and_save(edited_serde.clone().unwrap().to_string(), 
    modify_datapath(data_path.clone()), submit.filename.clone());
  if ret.is_err() { error!("modify_checklist compress_and_save"); return Err(ret.unwrap_err()); }

  // Update versioning if applicable. 
  let ret = upd_ver_temp(get_verpath(data_path.clone()),
    submit.filename.clone());
  if ret.is_err() { error!("modify_checklist upd_ver_temp"); return Err(ret.unwrap_err()); }
