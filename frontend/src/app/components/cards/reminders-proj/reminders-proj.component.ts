...
export class RemindersProjComponent {
  ...
  async get_pipeline_item_by_id() {
    ...
    this.http3.send("/pipeline/proj", JSON.stringify(row)).then(async (value: any) => {
      ...
      let answers = await this.http3.send("/response", JSON.stringify(row2));
  ...
  autoSave() {
    ...
    this.http3.send("/response/edit", JSON.stringify(row)).then(async (value: any) => {
